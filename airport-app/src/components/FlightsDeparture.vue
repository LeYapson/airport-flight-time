<template>
    <div class="container">
      ...
      <div v-if="!loading && filteredFlights.length === 0 && !error" class="alert alert-warning">
        ...
        {{ companyFilter ? 'Aucun vol ne correspond à votre recherche.' : 'Aucun vol au départ n\'est disponible pour le moment.' }}
      </div>
  ...
  const flightsListDeparture = ref([]);
  ...
  // Modifier la fonction filteredFlights comme ceci
  const filteredFlights = computed(() => {
    // Récupérer la liste des vols
    let flights = [...flightsListDeparture.value];
    ...
    // Trier par heure de départ avec prise en compte de l'ordre
  ...
  onMounted(async () => {
    try {
      ...
      /*
      const response = await fetch(`http://api.aviationstack.com/v1/flights?access_key=${apiKey}&dep_iata=${airportCode}`);
      if (!response.ok) {
      ...
      flightsListDeparture.value = data.data.map(flight => ({
        flight_number: flight.flight.iata || 'N/A',
        destination_airport_code: flight.arrival.iata || 'N/A',
        city: flight.arrival.airport || 'Information non disponible',
      ...
      */
      ...
      const response = await fetch('/data/flights-departure.json');
      if (!response.ok) {
      ...
      flightsListDeparture.value = data.flights.map(flight => ({
        flight_number: flight.flight_number || 'N/A',
        destination_airport_code: flight.destination_airport_code || 'N/A',
        city: flight.city || 'Information non disponible',
        company: flight.company || 'Information non disponible',
        departure_time: formatDateTimeFromString(flight.departure_time) || null,
